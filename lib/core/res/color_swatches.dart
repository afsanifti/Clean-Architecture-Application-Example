import 'dart:ui';

class ColorSwatches {
  const ColorSwatches._();

  static const gradient = [
    Color(0xFFEDF8FF),
    Color(0xFFFDC1E8),
    Color(0xFFFFFFFF),
    Color(0xFFFDFAE1),
  ];

  static const primaryColour = Color(0xFF458CFF);

  static const neutralTextColour = Color(0xFF757C8E);

  static const physicsFileColour = Color(0xFFD3D5FE);

  static const scienceFileColour = Color(0xFFFFEFDA);

  static const chemistryFileColour = Color(0xFFFFE4F1);

  static const biologyFileColour = Color(0xFFCFE5FC);

  static const mathFileColour = Color(0xFFFFCECA);

  static const mathTitleColour = Color(0xFFFFEEEA);

  static const languageTitleColour = Color(0xFFDAFFD6);

  static const literatureTitleColour = Color(0xFFD5BEFB);

  static const redColour = Color(0xFFFF5C5C);

  static const greenColour = Color(0xFF28CA6C);

  static const chatFieldColour = Color(0xFFF4F5F6);

  static const chatFieldColourDarker = Color(0xFFEBE9EA);

  static const currentUserChatBubbleColour = Color(0xFF2196F3);

  static const otherUserChatBubbleColour = Color(0xFFEEEEEE);

  static const currentUserChatBubbleColourDarker = Color(0xFF1976D2);

  static const otherUserChatBubbleColourDarker = Color(0xFFE0E0E0);
}
